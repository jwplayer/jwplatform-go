...
func (c *UploadsClient) ListUploadParts(uploadID string) (*UploadsListResponse, error) {
	path := fmt.Sprintf("/v2/uploads/%s/parts", uploadID)
	uploadParts := &UploadsListResponse{}
	err := c.v2Client.Request(http.MethodGet, path, uploadParts, nil, nil)
